...
function Leaderboard3({ params }) {
  ...
  const getUsers = async () => {
    const q1 = query(
      collection(db, "users"),
      orderBy("physicsRank"),
      where("physicsRank", ">", (parseInt(params.page) - 1) * 100),
      where("physicsRank", "<=", parseInt(params.page) * 100)
    );
            ...
            {pages.map((i) => {
              return (
                <>
                  ...
                  >
                    <Link className="page-link" href={`/problems/physics/leaderboard/${i}`}>
                      {i}
                    </Link>
                  </li>
                </>
