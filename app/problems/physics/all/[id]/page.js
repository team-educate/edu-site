  ...
  const handleAnswer = async (e) => {
    ...
    if (answer === "") {
      ...
      if (answer == problem.answer) {
        ...
        if (dt.data().physicsSolved.includes(problem.id)) {
          ...
          await updateDoc(doc(db, "users", user.uid), {
            submission: increment(1),
            physicsSolved: arrayUnion(problem.id),
            physicsUnsolved: arrayRemove(problem.id),
            physicsScore: increment(problem.point),
          });
          if (problem.solved == 0) {
            setRes(first)
            await updateDoc(doc(db, "physics", docID), {
        ...
        if (dt.data().physicsSolved.includes(problem.id)) {
          ...
          await updateDoc(doc(db, "users", user.uid), {
            submission: increment(1),
            physicsUnsolved: arrayUnion(problem.id),
          });
          await updateDoc(doc(db, "physics", docID), {
            tried: increment(1),
          });
