...
function LeaderboardMath() {
    ...
    const getUsers = async () => {
        const q1 = query(collection(db, 'users'), orderBy('mathRank'), where('mathRank', '>', (parseInt(params.page) - 1) * 100), where('mathRank', '<=', parseInt(params.page) * 100))
  ...
  return (
    <>
      <div className="container mt-5">
        <div className="card">
          <div className="card-header problem-header">Math Leaderboard</div>
        </div>
        <br />
                ...
                {users.map((user) => {
                    return (
                        <tr key={user.username}>
                            <th scope="row" className='fs-4'>{user.mathRank}</th>
                            ...
                            <td className='fs-4'>{user.mathScore}</td>
                        </tr>
                    )
                })}
              </tbody>
        ...
        <ul className="pagination justify-content-center">
          ...
          {pages.map(i => {
            return (
              <>
                <li key={i} className={`page-item ${i == params.page ? 'active' : ''} ${i < params.page - 2 || i > params.page + 2 ? 'd-none' : ''}`}><Link className="page-link" href={`/problems/math/leaderboard/${i}`}>{i}</Link></li>
              </>
            )
          })}
          <li className="page-item">
