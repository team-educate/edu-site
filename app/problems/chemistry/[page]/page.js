...
const getProblems = async (start) => {
  const q1 = query(collection(db, 'chemistry'), orderBy('id'), limit(50), startAfter(start))
...
const getUsers = async () => {
  const q1 = query(collection(db, 'users'), orderBy('chemistryRank'), limit(10))
  const docs = await getDocs(q1);
  const users = [];
  docs.forEach(doc => {
    users.push(doc.data())
        ...
        <div className="row">
          <div className="col-sm-6 mb-3 mb-sm-0">
              {problems.map(problem => {
                return(
                  <>
                    <Link key={problem.id} href={`/problems/chemistry/all/${problem.id}`} className="decno text-decoration-none text-dark">
                      <div className={`card ${userDoc.chemistrySolved.includes(problem.id) ? 'solved' : userDoc.chemistryUnsolved.includes(problem.id) ? 'unsolved' : 'notry'} card-dark`}>
          ...
          <div className="card top-5 position-sticky">
            <div className="card-body">
              ...
              <table className="table table-hover">
                ...
                <tbody>
                  {leaders.map(user => {
                    return (
                      <tr>
                        <th scope="row">{user.chemistryRank}</th>
                        <td>{'@' + user.username}</td>
              ...
              <p className="text-center">
                <Link href="/problems/chemistry/leaderboard" className=' text-decoration-none'>
      ...
      <nav aria-label="..." className="center">
        <ul className="pagination justify-content-center">
          ...
          {pages.map(i => {
            return (
              <>
                <li key={i} className={`page-item ${i == params.page ? 'active' : ''} ${i < params.page - 2 || i > params.page + 2 ? 'd-none' : ''}`}><Link className="page-link" href={`/problems/chemistry/${i}`}>{i}</Link></li>
              </>
            )
          })}
          <li className="page-item">
