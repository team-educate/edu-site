  ...
  const handleAnswer = async (e) => {
    ...
    if (answer === "") {
      ...
      if (answer == problem.answer) {
        ...
        if (dt.data().chemistrySolved.includes(problem.id)) {
          ...
          await updateDoc(doc(db, "users", user.uid), {
            submission: increment(1),
            chemistrySolved: arrayUnion(problem.id),
            chemistryUnsolved: arrayRemove(problem.id),
            chemistryScore: increment(problem.point),
          });
          if (problem.solved == 0) {
            setRes(first)
            await updateDoc(doc(db, "chemistry", docID), {
        ...
        if (dt.data().chemistrySolved.includes(problem.id)) {
          ...
          await updateDoc(doc(db, "users", user.uid), {
            submission: increment(1),
            chemistryUnsolved: arrayUnion(problem.id),
          });
          await updateDoc(doc(db, "chemistry", docID), {
            tried: increment(1),
          });
